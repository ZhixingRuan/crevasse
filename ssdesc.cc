 ...
 */
template<class fnumtype> void ssdesc::prune_normalise_ssdescs(vector<fnumtype>& ssdescs, const int image_width, const int image_height,	const ssdesc::ssdesc_parms<fnumtype> &parms, vector<fnumtype>* resp, vector<ssdesc::coordElem>* draw_coords, vector<ssdesc::coordElem>* salient_coords, vector<ssdesc::coordElem>* homogeneous_coords, vector<ssdesc::coordElem>* snn_coords, const ssdesc::rectRegion calc_rect)
{       
        int xfrom, xto, yfrom, yto;
...
extern "C"
{
	...
	void ssdesc_calc(const double* image, const int image_width, const int image_height, const int image_channels, const ssdesc::ssdesc_parms<double> parms, double* output)
	{
		vector<double> ssdescs;
		ssdesc::calc_ssdescs_alt(image, image_width, image_height, image_channels, parms, &ssdescs);

		std::copy(ssdescs.begin(), ssdescs.end(), output);
       	}
	void ssdesc_norm(vector<double>& ssdescs, const int image_width, const int image_height, const ssdesc::ssdesc_parms<double> &parms, double* resp_output)//, ssdesc::coordElem* draw_coords, ssdesc::coordElem* salient_coords, ssdesc::coordElem* homogeneous_coords, ssdesc::coordElem* snn_coords)
        {
		vector<double> resp;
                vector<ssdesc::coordElem> draw_co;
		vector<ssdesc::coordElem> salient_co;
		vector<ssdesc::coordElem> homogeneous_co;
		vector<ssdesc::coordElem> snn_co;

		ssdesc::prune_normalise_ssdescs(ssdescs, image_width, image_height, parms, &resp, &draw_co, &salient_co, &homogeneous_co, &snn_co);

                std::copy(resp.begin(), resp.end(), resp_output);
                //std::copy(draw_co.begin(), draw_co.end(), draw_coords);
                //std::copy(salient_co.begin(), salient_co.end(), salient_coords);
                //std::copy(homogeneous_co.begin(), homogeneous_co.end(), homogeneous_coords);
                //std::copy(snn_co.begin(), snn_co.end(), snn_coords);
	}
}
